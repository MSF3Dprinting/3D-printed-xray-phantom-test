...
`compute_all` turns that into ``{"status": "n/a", "error": ...}`` with no rows.
...
def test_a_failed_test_produces_no_rows_at_all():
    ...
    assert pipeline.overall_status(out) == "n/a"
...
def _record(results):
    return {"id": "abc123def456", "created_at": "2026-08-01 10:00:00",
            ...
            "status": "n/a", "is_baseline": 0, "validation_status": ""}
...
def test_step_e_does_not_count_not_analysed_as_passed():
    js = _app_js()
    assert 'const attention = cards.filter(cd => cd.status !== "pass");' in js, (
        'a status of "n/a" is the absence of an answer, not a pass')
